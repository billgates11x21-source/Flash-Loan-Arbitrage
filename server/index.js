      ...
      for (let i = 0; i < pairs.length; i++) {
        for (let j = i + 1; j < pairs.length; j++) {
          ...
          if (pair1.quoteToken.address === pair2.quoteToken.address) {
            const priceDiff = Math.abs(parseFloat(pair1.priceUsd) - parseFloat(pair2.priceUsd));
            const avgPrice = (parseFloat(pair1.priceUsd) + parseFloat(pair2.priceUsd)) / 2;
            const priceDiffPercent = (priceDiff / avgPrice) * 100;
            
            // Minimum 0.5% price difference for opportunity
